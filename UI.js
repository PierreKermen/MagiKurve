...
/* ── État global ── */
const selectedTypes = new Set(['shockland', 'checkland', 'fastland', 'basicland']);

/* ── Init : boutons de type de land ── */
function initToggles() {
  const container = document.getElementById('landToggles');
  const order = ['shockland','checkland','fastland','slowland','triome','basicland'];
...
async function calculate() {
  ...
  if (!text) {
    results.innerHTML = '<div class="error">Collez une decklist d\'abord.</div>';
    return;
  }

  const cards = parseDecklist(text);
  if (cards.length === 0) {
    results.innerHTML = '<div class="error">Aucune carte non-land détectée. Vérifiez le format (ex: "4 Llanowar Elves").</div>';
    return;
  }
  ...
  if (cardData.length === 0) {
    results.innerHTML = '<div class="error">Impossible de récupérer les données Scryfall. Vérifiez votre connexion.</div>';
    btn.disabled = false;
    setStatus(status, '');
    return;
  }
  ...
  /* ── Rendu ── */
  results.innerHTML = renderRequirements(requirements, landCount, cardData.length)
...
function renderRequirements(req, landCount, cardCount) {
  ...
  const cards = activeColors.map(c => `
...
function renderManaBases(manaBases, requirements) {
  ...
  let html = '<p class="section-label" style="margin-bottom:10px;">Mana bases proposées</p>';
...
function renderOneBase(mb, rank, requirements) {
  ...
  const satisfied     = activeColors.filter(c => mb.sources[c] >= requirements[c]).length;
  const isBest        = rank === 0;
  const badge         = isBest
  ...
  const sourceChecks = activeColors.map(c => {
